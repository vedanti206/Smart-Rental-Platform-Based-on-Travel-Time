...
// (React-Leaflet has a known issue with the default icon paths)
...
// Create custom colored icons if desired
...
const MapView = ({ properties, workplace }) => {
  ...
  return (
    <div className="w-full h-full rounded-2xl overflow-hidden shadow-sm border border-gray-200 z-10 relative">
      <MapContainer center={center} zoom={13} scrollWheelZoom={true} className="w-full h-full z-10">
        <TileLayer
        ...
        {properties.map(p => {
           ...
           const position = [p.location.coordinates[1], p.location.coordinates[0]];
           return (
             <Marker 
                key={p._id} 
                position={position}
             ...
             >
                <Popup>
                  <div className="p-1 min-w-[150px] font-sans">
                    ...
                    <p className="text-gray-600 font-bold">₹{p.price.toLocaleString()}<span className="text-xs text-gray-400 font-medium">/mo</span></p>
